...
UIKIT_EXTERN NSNotificationName const OpeUnitFieldDidBecomeFirstResponderNotification;
UIKIT_EXTERN NSNotificationName const OpeUnitFieldDidResignFirstResponderNotification;
#else
UIKIT_EXTERN NSString *const OpeUnitFieldDidBecomeFirstResponderNotification;
UIKIT_EXTERN NSString *const OpeUnitFieldDidResignFirstResponderNotification;
#endif

typedef NS_ENUM(NSUInteger, OpeKeyboardType) {
    OpeKeyboardTypeNumberPad,   // 纯数字键盘
    OpeKeyboardTypeASCIICapable // ASCII 字符键盘
};

@protocol OpeUnitFieldDelegate;

IB_DESIGNABLE

@interface OpeUnitField : UIControl

@property (nullable, nonatomic, weak) id<OpeUnitFieldDelegate> delegate;
...
#else
@property (nonatomic, assign) OpeKeyboardType defaultKeyboardType;  // 默认为JQKeyboardTypeNumberPad。
...
// 设置文本颜色，默认为黑色。
@property (null_resettable, nonatomic, strong) IBInspectable UIColor *palceholderColor;
...
@protocol OpeUnitFieldDelegate <NSObject>

@optional

- (BOOL)unitField:(OpeUnitField *)uniField shouldChangeCharactersInRange:(NSRange)range replacementString:(NSString *)string;
